    ...
    describe('File Logging', () => {
        ...
        it('should rotate log file when date changes', () => {
            ...
            (fs.existsSync as jest.Mock).mockImplementation((path: string) => {
                return path.includes('.ztk') && path.endsWith('ztk.log');
            });
      
            (fs.statSync as jest.Mock).mockImplementation((path: string) => {
                if (path.includes('ztk.log')) {
                    return { 
                        size: 1024,
                        mtime: yesterday
                    };
        ...
        it('should keep last 3 daily log files during rotation', () => {
            ...
            const oldFiles = [
                `ztk-${fileDate1.toISOString().split('T')[0]}.log`,
                `ztk-${fileDate2.toISOString().split('T')[0]}.log`,
                `ztk-${fileDate3.toISOString().split('T')[0]}.log`,
                `ztk-${fileDate4.toISOString().split('T')[0]}.log`, // Should be kept
                `ztk-${fileDate5.toISOString().split('T')[0]}.log`, // Should be kept
                `ztk-${fileDate6.toISOString().split('T')[0]}.log`  // Should be kept
            ];
      
            (fs.readdirSync as jest.Mock).mockImplementation((_path: string) => oldFiles);
      
            // Mock statSync for each file to return different dates
            (fs.statSync as jest.Mock).mockImplementation((filePath: string) => {
                if (filePath.endsWith('ztk.log')) return {mtime: yesterday};
